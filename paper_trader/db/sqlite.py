...
class _ClassDesc:
    ...
    def create_table_str(self):
        ...
        pks = 'PRIMARY KEY ({})'.format(', '.join(k for k in self.primary_keys_names))
    ...
    @property
    def delete_str(self):
        return f'DELETE FROM {self.name} WHERE ' + ' AND '.join(f'{pk}=?' for pk in self.primary_keys_names)
...
class SqliteDb:
    ...
    def get_all(self, clazz):
        ...
        for vals in self._db.cursor().execute(desc.get_all_str).fetchall():
            yield clazz(*(_convert_val_from_db(t, v) for t, v in zip(desc.field_types, vals)))
